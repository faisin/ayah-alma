#!/bin/bash
# Hapus Akun Trojan - by Ayah Alma
...
if [[ ! -s $DB ]]; then
    echo -e "${RED}Tidak ada user Trojan terdaftar!${NC}"
    echo ""
...
if ! jq --arg user "$user" '
(.inbounds[] | select(.tag=="trojan-ws-tls").settings.clients) |= [ .[] | select(.email != $user) ] |
(.inbounds[] | select(.tag=="trojan-grpc").settings.clients) |= [ .[] | select(.email != $user) ]
' "$CONFIG" > "$tmpfile"; then
...
if ! xray -test -config "$CONFIG" >/dev/null 2>&1; then
    echo -e "${RED}❌ Uji konfigurasi Xray gagal! Memulihkan...${NC}"
